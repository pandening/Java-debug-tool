...
package io.javadebug.core.monitor.cpu;
...
public enum CPUCollector implements MonitorCollector {
    CPU_COLLECTOR
    ;
    ...
    public CounterEvent collect() {
        return new CounterEvent() {
            @Override
            public String type() {
                return "InCompletable Event Type";
            }

            @Override
            public String event() {
                return "use 'load'/'thread' type to watch cpu\n";
            }
        };
    }
}
