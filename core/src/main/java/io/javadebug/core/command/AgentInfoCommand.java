...
@CommandDescribe(
        name = "info",
        simpleName = "if",
        function = "获取代理相关运行时信息，比如连接数量，处理进度，被剔除的连接等",
        usage = "info -p [info-name]",
        cmdType = CommandType.COMPUTE
)
public class AgentInfoCommand implements Command {

    //--------------------------------------------------
    /// 当前支持的所有获取agent信息的参数
    //--------------------------------------------------
    private static final Map<String, Function<String, String>> INFO_COMMAND_MAP = new HashMap<>();

    static {

        /// 获取当前服务端连接的所有连接信息（不要乱用）
        INFO_COMMAND_MAP.put("cons", new Function<String, String>() {
            @Override
    ...
    public boolean preExecute(RemoteCommand remoteCommand) throws Exception {
        String infoP = remoteCommand.getParam("$forward-info-p");
        if (UTILS.isNullOrEmpty(infoP)) {
            return false;
        }
        return true;
    }
    ...
    public String execute(Instrumentation ins, RemoteCommand reqRemoteCommand, CommandServer commandServer, ServerHook serverHook) throws Exception {
        String infoP = reqRemoteCommand.getParam("$forward-info-p");

        Function<String, String> function = INFO_COMMAND_MAP.get(infoP);
        if (function == null) {
            return "不支持的信息项:" + infoP + "\n";
        }
        return function.apply(infoP);
    }
