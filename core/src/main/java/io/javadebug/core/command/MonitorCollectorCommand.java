...
import io.javadebug.core.transport.NettyWebSocketTransportServer;
...
@CommandDescribe(
        name = "monitor",
        simpleName = "collect",
        function = "set up the monitor server",
        usage = "monitor / collect, the client should check the resp,\n" +
                        " if the resp is 'ok', then continue,\n" +
                        " if the resp is 'error', do not try to connect webSocket server\n",
        cmdType = CommandType.COMPUTE
)
public class MonitorCollectorCommand implements Command {
    ...
    public String execute(Instrumentation ins, RemoteCommand reqRemoteCommand, CommandServer commandServer, ServerHook serverHook) throws Exception {
        ...
        // set the port
        String portVal = reqRemoteCommand.getParam("$forward-monitor-port");
        int port = UTILS.safeParseInt(portVal, 20234);

        // try to set up the webSocket server
        try {
            NettyWebSocketTransportServer.NETTY_WEB_SOCKET_TRANSPORT_SERVER.initWebSocketServer(port);
        } catch (Exception e) {
            PSLogger.error(e.getMessage(), e);
            return e.getMessage();
        }

        // check and resp
        if (NettyWebSocketTransportServer.NETTY_WEB_SOCKET_TRANSPORT_SERVER.isStarted()) {
            return "ok";
        } else {
            return "error";
        }
